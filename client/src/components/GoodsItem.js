...
const GoodsItemt = ({goods}) => {
  ...
  return(
    <div className='goods-item' onClick={() => handleClick(goods.id)}>
      ...
      <div className='price'>
          {goods.discount ? <div className='discount-price'><span>{goods.price}</span>грн</div> :  <div className='standart-price black'><span>{goods.price}</span>грн</div> }
          {goods.discount ? <div className='standart-price'><span>{discount}</span>грн</div> : null }
      </div>
      {goods.quantity === 0 ? <div className='not-available'>Немає в наявності</div> : null }
